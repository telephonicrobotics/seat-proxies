...
: A TLS-terminating proxy that forwards the signalling defined in this
...
: A Key Encapsulation Mechanism key pair generated fresh by the client
  for a single session and never reused, used as the recipient key for
  HPKE Base Mode (Mode 0) {{RFC9180}} operations.

Origin KEM key (pkR):
: The origin's KEM public key, distributed in an Identity Document and
  authenticated by the client prior to the handshake as described in the
  Origin Key Provisioning section.
...
: Ephemeral random values exchanged between client and origin under
  HPKE. cnC is generated by the client and sealed to pkR; cnR is
  generated by the origin and sealed to pkE.
...
: A symmetric secret derived from cnC and cnR.  It is injected into
  ...
  the intermediary from the session.
...
by that document is shown as `...`.  In the diagram, `[EA §6
extensions]` denotes the attestation negotiation extensions defined in
Section 6 of
{{I-D.fossati-seat-early-attestation}}.
...
derived from `main_secret_N+1`, which is a function of `combined_ikm`
and therefore of `psk_attest`.  An entity that cannot derive `cnR`
cannot derive `psk_attest`, cannot compute `main_secret_N+1`, and cannot
...
By incorporating the respective ephemeral KEM public key into the
...
Evidence is delivered during the TLS handshake via the `attestation`
extension in the `Certificate` message, as defined in
{{I-D.fossati-seat-early-attestation}}. However, to ensure the
...
Immediately upon completion of the TLS handshake, the client initiates
an Extended Key Update {{I-D.ietf-tls-extended-key-update}} that injects
`psk_attest` into the key schedule.  This rotates the application
traffic keys to values that incorporate attestation-derived keying
material inaccessible to the intermediary.
...
This document therefore mandates a key schedule reset: the
`HKDF-Extract` salt MUST be set to 0, severing the chain to the
proxy-terminated session, and the input keying material MUST be the
concatenation of `eku_dh_shared` and `psk_attest`.  A zero salt
instructs the key schedule to treat this rotation as a fresh derivation
anchored entirely in `combined_ikm`, analogous to how TLS 1.3 derives
its Early Secret from a PSK with no prior session state.
...
Because the TLS-terminating intermediary breaks the native TLS
...
instead.  `aad_ct` binds `ctC` to `pkR`, the ClientHello random, and the
negotiated parameters, preventing replay to a different origin or
session.  `aad_ee` is `Hash(cnC)`: because `cnC` was HPKE-sealed to
`pkR` and is never transmitted in the clear, the intermediary cannot
observe it, making `aad_ee` opaque to the proxy despite the severed
...
requiring explicit detection of adversarial intent.
...
In proxy-fronted topologies, the TLS handshake is terminated by the
...
valid-looking Evidence bound to attacker-controlled KEM parameters.
...
The `nonce_challenge_ext` extension is permitted in the ClientHello (CH)
